<tr>
    ...
    <td>
        @php
            ...
            $result = '';
            if($date_now < $date2){
                $result.='<div>'.\Carbon\Carbon::parse($date_time)->diffForHumans().'</div>';
            } else{
                $result.=  $date_time . ' '. $project->timezone;
            }
        @endphp
        <span class="text-xs md:text-sm lg:text-base uppercase">{!!  $result !!}</span>
    </td>
    <td class="hidden md:flex items-center">
        <div class="flex items-center ">
            @if($project->website_link != null)
            <a href="{{ $project->website_link }}" target="_blank"><i class="fas fa-globe-americas p-1 text-indigo-600 text-2xl"></i></a>
            @endif
            @if($project->opensea_link != null)
            <a href="{{ $project->opensea_link }}" target="_blank">   <img class="w-8 h-8 p-1" src="{{ asset('images/opensea.svg') }}" alt=""/></a>
            @endif
            @if($project->discord_link != null)
            <a href="{{ $project->discord_link }}" target="_blank" ><i class="fab fa-discord p-1 text-indigo-600 text-2xl"></i></a>
            @endif
            @if($project->twitter_link != null)
            <a  href="{{ $project->twitter_link }}"  target="_blank"    ><i class="fab fa-twitter p-1 text-indigo-600 text-2xl "></i></a>
            @endif
        </div>
    </td>
    <td class="sm:w-64 md:w-72">
        <div class="flex flex-wrap items-center justify-center">
            ...
            @if(auth()->check())
                @if($project->user_id != auth()->user()->id)
                    <a  href="{{ route('add.vote', $project->id) }}"  class="m-0.5 sm:m-1  py-2 px-2 border border-yellow-400 text-yellow-600 hover:bg-yellow-400 rounded hover:text-white transition-all duration-200 text-xs sm:text-sm md:text-base flex justify-center w-full sm:w-auto">Vote <span class="pl-1">{{  Helper::shortNumber($project->custom_vote_sum_vote != null ?  $project->custom_vote_sum_vote : 0) }}</span></a>
                @else
                <p   class="m-0.5 sm:m-1  py-2 px-2 border border-yellow-400 text-yellow-600   rounded  text-xs sm:text-sm md:text-base flex justify-center w-full sm:w-auto">Vote <span class="pl-1"> {{  Helper::shortNumber($project->custom_vote_sum_vote != null ?  $project->custom_vote_sum_vote : 0) }}</span></p>
                @endif
            @else
                <a  href="{{ route('add.vote', $project->id) }}"  class="m-0.5 sm:m-1  py-2 px-2 border border-yellow-400 text-yellow-600 hover:bg-yellow-400 rounded hover:text-white transition-all duration-200 text-xs sm:text-sm md:text-base flex justify-center w-full sm:w-auto">Vote  <span class="pl-1"> {{ $project->custom_vote_sum_vote != null ?  $project->custom_vote_sum_vote : 0 }}</span></a>
            @endif
